package com.example.fruitapp.ui.fruitedit
...
import com.example.fruitapp.databinding.ActivityEditBinding
...
class EditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.binding = DataBindingUtil.setContentView(this,R.layout.activity_edit)
        this.binding.fruit=
            com.example.fruitapp.presmodels.Fruit(null,"", "", "", "", 0.0, 0.0, 0.0, 0, 0.0)
        lifecycleScope.launch(Dispatchers.Main){
            val name2 = viewModel.GetFruit(name)
            binding.fruit=name2

        }

    }
    fun editNewFruit(view: View){
        val fruit= createFruitFromForm()
        Log.println(Log.WARN,"TestLog",fruit.toString())
        viewModel.UpdateFruit(fruit)
        finish()
    }
    private fun createFruitFromForm(): Fruit {
        ...
        fruit= Fruit(binding.fruit?.id,name,genus,family,order,carbohydrates,protein ,fat  ,calories,sugar )
